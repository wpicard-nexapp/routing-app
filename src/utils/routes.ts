...
export function resolve<R extends Routes>(routes: R, prefix = '') {
  return map(routes, (node, parent) => join(parent ?? '', node), prefix);
}
...
//     ...routes,
//     node: root,
//     childs: Object.entries(routes.childs).reduce<Childs>((acc, [key, subTree]) => ({
//       ...acc,
